...
def run_experiment(
    ...
    for grouped_data_items in grouped_dataset:
        ...
        if use_cache and is_group_cached:
            # Already inferenced example(s)
            batch_prediction, batch_gold_answer = [], []
            for idx, data_item in grouped_data_items:
                with open(
                    os.path.join(tmp_save_dir, f"{data_item['idx']}_{data_item['id']}.json"), "r"
                ) as f:
...
if __name__ == "__main__":
    ...
    group_file = f"./misc/group_results/{dataset_name}-batch={num_in_batch}-{group_method}.json"
    ...
    tmp_save_dir = os.path.join(save_dir, "tmp", f"{exp_name}-{group_method}/")
